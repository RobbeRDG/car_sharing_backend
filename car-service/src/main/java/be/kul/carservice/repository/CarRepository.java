...
public interface CarRepository extends JpaRepository<Car, Long> {
    ...
    @Query(value = "SELECT * FROM cars.car as c WHERE ST_Distance(c.location, ST_SRID(POINT( :userLongitude, :userLatitude ), 4326)) < ( :radiusInKM * 1000) and in_maintenance=false and c.ride_id is null and c.reservation_id is null ORDER BY ST_Distance(c.location, ST_SRID(POINT( :userLongitude, :userLatitude ), 4326))"
            , nativeQuery = true)
